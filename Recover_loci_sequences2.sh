...
# 1) Create a bed file
cat $3 | while read line; do python3 $2Create_bed_file.py --table $1${line}/Matches_${line}_without_overlapping_sequences.m8 --out $1${line}/Recover_fasta_loci.bed; echo $'\n';
done 

# 2)  Get the fasta sequences from the genomes according to the coordinates from the bed file. 
